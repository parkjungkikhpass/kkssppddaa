...
/**
 * ElevenLabs API에서 사용 가능한 음성 목록을 가져옵니다.
 */
export const fetchElevenLabsVoices = async (apiKey?: string): Promise<ElevenLabsVoice[]> => {
  ...
  const finalKey = apiKey || savedApiKey;

  console.log('[ElevenLabs] 음성 목록 요청, API Key 존재:', !!finalKey, finalKey ? `(${finalKey.slice(0, 8)}...)` : '');

  if (!finalKey || finalKey.length < 10) {
    console.warn("[ElevenLabs] API Key가 설정되지 않았거나 너무 짧습니다.");
    return [];
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error("[ElevenLabs] 음성 목록 불러오기 실패:", response.status, errorText);
      return [];
    }

    const data = await response.json();
    console.log('[ElevenLabs] 음성 목록 로드 성공:', data.voices?.length || 0, '개');
    return data.voices || [];
  } catch (error) {
    console.error("[ElevenLabs] 음성 목록 불러오기 실패:", error);
    return [];
  }
};
...
export const generateAudioWithElevenLabs = async (
  ...
  try {
    ...
    if (jsonResponse.alignment) {
      ...
      console.log(`[ElevenLabs] 자막 데이터 생성 완료: ${words.length}개 단어, 추정 길이: ${estimatedDuration?.toFixed(2)}초`);
