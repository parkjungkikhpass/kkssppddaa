...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, onClick, onDelete, onDuplicate, isSelectMode, isSelected, onSelect }) => {
  ...
  return (
    <div
      ...
      className={`group relative bg-white border-2 rounded-2xl p-4 cursor-pointer transition-all duration-200 shadow-md ${
        isSelectMode
          ? isSelected
            ? 'border-brand-500 bg-brand-50 ring-2 ring-brand-500/30'
            : 'border-gray-200 hover:border-gray-300'
          : 'border-gray-200 hover:border-brand-400 hover:shadow-xl hover:-translate-y-1'
      }`}
    >
      {/* 선택 모드 체크박스 */}
      {isSelectMode && (
        ...
        <div className="absolute top-3 right-3 flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-all z-10">
          ...
          {onDuplicate && (
            <button
              ...
              className="p-2 rounded-xl bg-white/90 text-gray-400 hover:bg-brand-50 hover:text-brand-500 transition-all shadow-sm"
              title="프로젝트 복제"
            >
          ...
          {onDelete && (
            <button
              ...
              className="p-2 rounded-xl bg-white/90 text-gray-400 hover:bg-red-50 hover:text-red-500 transition-all shadow-sm"
              title="프로젝트 삭제"
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {project.thumbnail && !imageError ? (
        <div className="mb-4 rounded-xl overflow-hidden aspect-video bg-gray-100 relative">
          <img
        ...
        <div className="mb-4 rounded-xl aspect-video bg-gradient-to-br from-gray-50 to-gray-100 flex flex-col items-center justify-center relative">
          ...
          <span className="text-xs text-gray-400">썸네일 없음</span>
          {/* 상태 뱃지 */}
          <div className="absolute top-2 left-2">
      ...
      {/* 제목 */}
      <h3 className="text-gray-900 font-semibold text-base mb-3 line-clamp-2 leading-snug group-hover:text-brand-600 transition-colors">
        {project.title}
      </h3>
      ...
      <div className="flex items-center justify-between pt-3 border-t-2 border-gray-100">
        <div className="flex items-center gap-3 text-xs text-gray-400">
          ...
          {/* 스타일 */}
          {styleInfo && (
            <span className="text-gray-400">{styleInfo.emoji}</span>
          )}
        </div>
        ...
        {project.status === 'in_progress' ? (
          ...
          <svg className="w-4 h-4 text-gray-300 group-hover:text-brand-500 group-hover:translate-x-1 transition-all" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export const NewProjectCard: React.FC<{ onClick: () => void }> = ({ onClick }) => {
  return (
    <button
      onClick={onClick}
      className="group bg-gradient-to-br from-gray-50 to-gray-100 border-2 border-dashed border-gray-400 rounded-2xl p-4 cursor-pointer hover:border-brand-500 hover:border-solid hover:from-brand-50 hover:to-brand-100/50 transition-all duration-300 flex flex-col items-center justify-center min-h-[280px] shadow-sm hover:shadow-md"
    >
      <div className="w-14 h-14 rounded-2xl bg-gray-200 group-hover:bg-brand-100 group-hover:scale-110 flex items-center justify-center mb-4 transition-all duration-300">
        <svg className="w-7 h-7 text-gray-400 group-hover:text-brand-500 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
        </svg>
      </div>
      <span className="text-gray-600 group-hover:text-gray-900 font-semibold text-sm transition-colors mb-1">
        새 프로젝트
      </span>
